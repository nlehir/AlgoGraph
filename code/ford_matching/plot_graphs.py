...
def build_residual_graph(G, nodes_1, nodes_2, residual_capacities, capacities):
    G_residual = G.copy()
...
def show_residual_network_nx(
    G_residual, pos, residual_capacities, capacities, nodes_1, nodes_2, dir_name, step
):
    ...
    for edge in G_residual.edges:
        ...
        if edge[0] == "Source":
            ...
            node_group_2 = int(edge[0].split("-")[1])
            node_group_1 = int(edge[1].split("-")[1])
            residual_capacity = residual_capacities[node_group_2 + 1, node_group_1 + 1]
        else:
            node_group_1 = int(edge[0].split("-")[1])
            node_group_2 = int(edge[1].split("-")[1])
            residual_capacity = residual_capacities[node_group_1 + 1, node_group_2 + 1]
        if residual_capacity != 0:
            if reverse:
                edges_width.append(1)
